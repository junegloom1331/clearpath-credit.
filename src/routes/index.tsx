...
function LeadForm() { const [sent,setSent]=useState(false); const [error,setError]=useState(""); async function submit(e: React.FormEvent<HTMLFormElement>) { e.preventDefault(); const fd=new FormData(e.currentTarget); try { await saveLead({data:{name:fd.get("name"),email:fd.get("email"),note:fd.get("note")}}); setSent(true); } catch { setError("Please check your name and email, then try again."); } } if(sent) return <div className="success" role="status" aria-live="polite"><span>✳</span><h3>You’re on the list.</h3><p>We’ll be in touch when the next clear step is ready.</p></div>; return <form className="lead-form" onSubmit={submit}><label>YOUR NAME<input name="name" required placeholder="First and last"/></label><label>EMAIL ADDRESS<input name="email" type="email" required placeholder="you@example.com"/></label><label>WHAT DO YOU WANT TO FIX? <small>OPTIONAL</small><textarea name="note" rows={3} placeholder="A collection, an old address, a mystery account…"/></label>{error&&<p className="error" role="alert">{error}</p>}<button className="button button-light" type="submit">Keep me posted <span>↗</span></button><p className="form-fine">By signing up, you agree to receive ClearPath updates. Unsubscribe anytime.</p></form>; }
